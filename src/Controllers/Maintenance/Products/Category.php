...
class Category extends PublicController
{
    ...
    {
        // Validación del ID (requerido en todos los modos excepto Insertar)
        if ($this->viewData["mode"] !== "INS" && !isset($_POST["id"])) {
            $this->throwError(
                "Something went wrong, try again.",
        ...
        // Validación del token XSR (siempre requerido)
        ...
        // Validación de consistencia del ID (excepto en INS)
        if (
            $this->viewData["mode"] !== "INS" &&
            intval($_POST["id"]) !== $this->viewData["id"]
        ) {
            $this->throwError(
                "Something went wrong, try again.",
                "Inconsistent ID value. Expected: " . $this->viewData["id"] . ", Received: " . $_POST["id"]
            );
        }

        // Validación del token XSR
        if ($_POST["xsrtoken"] !== $_SESSION[$this->name . "-xsrtoken"]) {
            $this->throwError(
                "Something went wrong, try again.",
                "Invalid XSRToken. Expected: " . $_SESSION[$this->name . "-xsrtoken"] . ", Received: " . $_POST["xsrtoken"]
            );
        }
        ...
        // Solo asignar ID si no es INS (Insert)
        if ($this->viewData["mode"] !== "INS") {
